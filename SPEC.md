...
| F1-04 | C. 장바구니 (필터·저장자 표시) | [x] | 멤버/카테고리 필터 동작 |
| F1-05 | D. 개인 취향 프로필 + 변화 하이라이트 | [x] | 숏폼 3건 저장 시 점수 누적이 보임, 원점수 미노출 |
| F1-06 | 로딩/빈/에러 상태, 모바일 점검 | [x] | 4장 모든 FE1 화면 적용 |
...
| ID | 작업 | 상태 | 완료 기준 |
|---|---|---|---|
| F2-01 | A. 랜딩 + 여행방 생성 폼 | [x] | 생성 후 홈으로 이동, 입력 검증 동작 |
| F2-02 | B. 레이아웃 + 탭 + 여행방 홈 | [x] | FE1 화면이 탭으로 연결됨 |
| F2-03 | C. 그룹 취향 매트릭스 | [x] | 3인×카테고리 값 표시 |
| F2-04 | C. 공통 vs 개인 고유 구분 + 해석 문구 + CTA | [x] | 카페=공통, 쇼핑=민수로 구분 |
| F2-05 | D. 일정 조건 입력 폼 | [x] | 필수 장소 선택 포함 |
| F2-06 | D. 단계 로딩 UI + 생성 호출 | [x] | 파이프라인 문구 순차 표시 |
| F2-07 | D. 타임라인 카드 + 추천 이유 + 반영 사용자 칩 | [x] | 모든 항목에 reason·relatedUsers 표시 |
| F2-08 | D. Kakao 지도 마커·연결선·카드 연동 | [x] | 마커 클릭 시 카드 포커스 (키 없으면 지도만 비활성) |
| F2-09 | D. 사람별 반영도 패널 + 경고/보정 뱃지 | [x] | 서버 값만 표시 |
| F2-10 | D. 다시 생성, 재진입 복원, 로딩/빈/에러, 모바일 점검 | [x] | 새로고침 후 일정 복원 |
